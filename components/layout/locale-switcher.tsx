...
export function LocaleSwitcher({ invert = false }: { invert?: boolean }) {
  return (
    <Suspense fallback={<LocaleSwitcherFallback invert={invert} />}>
      <LocaleSwitcherSelect invert={invert} />
    </Suspense>
  );
}

function LocaleSwitcherFallback({ invert }: { invert: boolean }) {
...
function LocaleSwitcherSelect({ invert }: { invert: boolean }) {
  const locale = useLocale();
